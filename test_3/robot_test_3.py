...
for i in range(2):
    #画像の変数宣言
    pic_name="robot_test_3_"+str(i+1)+ ".jpg"
    pic_name_out="robot_test_3_"+str(i+1)+ "out.jpg"
    gray_pic="robot_test_3_"+str(i+1)+ "gray.jpg"
    ...
    #ベース画像と同じ大きさの配列を作成
    img_robot_test_3_=np.zeros((height,width,3),np.uint8)
    
    #紫色を指定
    img_robot_test_3_[(h > 179) & (s > 100)& (v > 100)] = 255
    
    #紫の領域だけの画像を作成
    cv2.imwrite(gray_pic,np.array(img_robot_test_3_))
