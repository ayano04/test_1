...
for i in range(4):
    #画像の変数宣言
    pic_name="robot_test_5_"+str(i+1)+ ".jpg"
    pic_name_out="robot_test_5_"+str(i+1)+ "out.jpg"
    gray_pic="robot_test_5_"+str(i+1)+ "gray.jpg"
    ...
    #ベース画像と同じ大きさの配列を作成
    img_robot_test_5_=np.zeros((height,width,3),np.uint8)
    
    #ロボットを認識
    img_robot_test_5_[(h > 179) & (s > 100)& (v > 100)] = 255
    #二値化画像を作成
    cv2.imwrite(gray_pic,np.array(img_robot_test_5_))
    #作成した画像を読み込み
    ...
    #ターゲットを認識
    img_robot_test_5_[(h > 70) & (h < 80) & (s > 100) & (v > 100)] = 255
    #二値化画像を作成
    cv2.imwrite(gray_pic,np.array(img_robot_test_5_))
    #作成した画像を読み込み
